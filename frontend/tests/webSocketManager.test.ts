...
import { ApiError, type SendMessageRequest } from '../src/shared/api/client.ts'
...
test('authenticates in the first frame and routes only authenticated events and correlated replies', async () => {
  ...
  socket.receive(null)
  socket.receive({ type: 'message.new', data: { id: 'recipient-envelope' } })
  assert.deepEqual(received, [{ id: 'recipient-envelope' }])
  ...
  socket.receive({ type: 'message.accepted', request_id: frame.request_id, data: { id: 'accepted' } })
  assert.equal((await sending).id, 'accepted')
  manager.stop()
})
...
test('preserves server rejection codes for the existing delivery-target retry', async () => {
  ...
  await assert.rejects(sending, (error: unknown) => error instanceof ApiError && error.status === 409 && error.code === 'delivery_targets_changed')
  manager.stop()
})
