import { ApiError, apiClient, type MailboxEnvelope, type SendMessageRequest, type SentMessage } from './client.ts'
...
export class WebSocketManager implements RealtimeTransport {
  ...
  sendMessage(command: SendMessageRequest): Promise<SentMessage> {
    ...
    return new Promise((resolve, reject) => {
      const timer = window.setTimeout(() => {
        ...
        reject(new ApiError('Message confirmation timed out. Delivery is unconfirmed.', 0))
      }, 15_000)
      this.pending.set(requestId, { resolve, reject, timer })
      try {
        this.socket!.send(JSON.stringify({ type: 'message.send', request_id: requestId, data: command }))
      } catch {
        window.clearTimeout(timer)
        this.pending.delete(requestId)
        reject(new ApiError('Connection lost. Message delivery is unconfirmed.', 0))
      }
    })
  }
  ...
  private async connect(refresh: boolean): Promise<void> {
    ...
    try {
      ...
      socket.onopen = () => {
        if (current()) socket.send(JSON.stringify({ type: 'auth', access_token: accessToken }))
      }
      socket.onmessage = (event) => {
        if (!current() || typeof event.data !== 'string') return
        let frame: ServerEvent
        try { frame = JSON.parse(event.data) as ServerEvent } catch { return }
  ...
  private rejectPending(): void {
    for (const pending of this.pending.values()) {
      window.clearTimeout(pending.timer)
      pending.reject(new ApiError('Connection lost. Message delivery is unconfirmed.', 0))
    }
    this.pending.clear()
  }
}

export const webSocketManager = new WebSocketManager(apiClient)
